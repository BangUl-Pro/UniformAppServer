...
io.on('connection', function(socket) {
	...
	socket.on('getMySchoolRanking', function(data) {
		...
		mySqlConnection.query('SELECT A FROM (SELECT @a:=@a+1 AS ROWNUM FROM schools, (SELECT @a:=0) ORDER BY school_point) AS A, schools WHERE schools.schoolId = ' + schoolId +';', function(err, result) {
			if (err) {
